        ...
        if table_content:
            ...
            if main_div:
                ...
                if print_results_div:
                    ...
                    doc_count_element = print_results_div.find("li", class_="sce-listitem-inline")
                    if doc_count_element:
                        text = doc_count_element.get_text(strip=True)
                        if "Document Count:" in text:
                            doc_count = text.replace("Document Count:", "").strip()
                            print(doc_count)
...
def get_document_count(session, document_link="Results.aspx"):
    base_url = "https://www.thecountyrecorder.com/"
    ...
    if response.status_code == 200:
        ...
        # Find the <li> element containing the document count
        doc_count_element = soup.find("li", class_="sce-listitem-inline", style="width: 15%;")
        if doc_count_element:
            text = doc_count_element.get_text(strip=True)
            if "Document Count:" in text:
                return text.replace("Document Count:", "").strip()
